...
class Odf
{
    ...
    {
    	if (! is_array($config)) {
    		throw new OdfException('Dados de configuração devem ser passados como array.');
    	}
        ...
        if (! class_exists($this->config['ZIP_PROXY'])) {
            throw new OdfException($this->config['ZIP_PROXY'] . ' classe não encontrada - cheque as configurações do php.');
        }
        $zipHandler = $this->config['ZIP_PROXY'];
        $this->file = new $zipHandler();
        if ($this->file->open($filename) !== true) {
            throw new OdfException("Erro ao abrir o arquivo '$filename' - cheque seu arquivo odt");
        }
        if (($this->contentXml = $this->file->getFromName('content.xml')) === false) {
            throw new OdfException("Nada para interpretar - verifique se o arquivo content.xml está formatado corretamente.");
        }
    ...
    public function setVars($key, $value, $encode = true, $charset = 'ISO-8859')
    {
        if (strpos($this->contentXml, $this->config['DELIMITER_LEFT'] . $key . $this->config['DELIMITER_RIGHT']) === false) {
            throw new OdfException("variável $key não encontrada no documento");
        }
    ...
    public function setImage($key, $value)
    {
        ...
        if ($size === false) {
            throw new OdfException("Imagem inválida");
        }
...
IMG;
    ...
    public function mergeSegment(Segment $segment)
    {
        if (! array_key_exists($segment->getName(), $this->segments)) {
            throw new OdfException($segment->getName() . 'não pode ser tratada, ela já foi definida ?');
        }
    ...
    public function setSegment($segment)
    {
        ...
        if (preg_match($reg, html_entity_decode($this->contentXml), $m) == 0) {
            throw new OdfException("'$segment' segmento não encontrado no documento");
        }
    ...
    public function saveToDisk($file = null)
    {
        if ($file !== null && is_string($file)) {
        	if (file_exists($file) && !(is_file($file) && is_writable($file))) {
            	throw new OdfException('Permissão negada : não pode criar ' . $file);
        	}
            $this->_save();
    ...
    private function _save()
    {
        ...
        if (! $this->file->addFromString('content.xml', $this->contentXml)) {
            throw new OdfException('Erro durante a exportação do arquivo');
        }
    ...
    public function exportAsAttachedFile($name="")
    {
        ...
        if (headers_sent($filename, $linenum)) {
            throw new OdfException("cabeçalho já foi enviado ($filename at $linenum)");
        }
