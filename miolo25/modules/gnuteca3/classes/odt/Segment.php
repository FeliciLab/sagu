...
class Segment implements IteratorAggregate, Countable
{
    ...
    public function __construct($name, $xml, $odf)
    {
        ...
        $this->xml = $this->preparaVariables((string) $xml);
		$this->odf = $odf;
    ...
    /**
     * Parse the variables and replace value with lower value.
     * 
     * @param String $content Content.
     * @return String content.
     */
    public function preparaVariables($content)
    {
        ...
        if ( is_array($content) )
        {
            ...
            {
                $variable = substr($value, 0, stripos($value, '<'));
                $variables[$variable] = strtolower($variable);
            }
        }
        
        $content = str_replace(array_keys($variables), array_values($variables), $originalContent);
    ...
    {
        if (strpos($this->xml, $this->odf->getConfig('DELIMITER_LEFT') . strtolower($key) . $this->odf->getConfig('DELIMITER_RIGHT')) === false) {
            throw new SegmentException("var $key not found in {$this->getName()}");
        }
		$value = $encode ? htmlspecialchars($value) : $value;
		$value = ($charset == 'ISO-8859') ? utf8_encode($value) : $value;
        $this->vars[$this->odf->getConfig('DELIMITER_LEFT') . strtolower($key) . $this->odf->getConfig('DELIMITER_RIGHT')] = str_replace("\n", "<text:line-break/>", $value);
        return $this;
    }
    /**
...
IMG;
        ...
        $this->setVars(strtolower($key), $xml, false);
        return $this;
    }	
    /**
