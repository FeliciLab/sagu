CREATE OR REPLACE FUNCTION cr_fin_financiamento(p_periodid varchar, p_contractid integer, p_incentivetypeid integer)
  RETURNS TABLE (  tipo_incentivo_cod integer,
                   ...
                   pessoa_cod bigint,
                   pessoa_nome varchar,
                   total_valor_financiado numeric,
                   total_valor_financiado_pago numeric) AS
$BODY$
/*************************************************************************************
...
BEGIN
    ...
    v_select:= 'SELECT w1.incentivetypeid,
                       ...
                       w1.personid,
                       w1.name,         
                       SUM(w1.valor) as valor,
                       SUM(w1.valorpago) as valorpago
                  FROM (SELECT a.incentivetypeid,
                               ...
                               f.name, 
                               (SELECT sum(s2.value) 
                                  FROM ONLY fininvoice s1
                                 INNER JOIN finentry s2
                                         ON s1.invoiceid = s2.invoiceid
                                      WHERE s2.titulodereferencia = e.invoiceid AND s1.iscanceled = false) as valor,
                                    (SELECT (s11.value - balance(s11.invoiceid)) as valor 
                                       FROM ONLY fininvoice s11
                                      WHERE s11.invoiceid = (SELECT MAX(S22.invoiceid) 
                                                               FROM finentry s22
                                                              WHERE s22.titulodereferencia = e.invoiceid) AND s11.iscanceled = false)as valorpago
                          FROM finloan a
                         ...
                         WHERE d.periodid = '''|| p_periodid ||'''
                           ...
                           AND e.invoiceid IN (SELECT x4.invoiceid
                                           ...
                                           INNER JOIN finentry x5
                           	                    ...
                           	                    WHERE x5.incentivetypeid = a.incentivetypeid)
                           --verifica se tem titulos referenciados
                           AND e.invoiceid IN (SELECT COALESCE(x1.titulodereferencia, x2.titulodereferencia)
                                                    FROM fininvoice x1
                                               LEFT JOIN finentry x2 
                                                      ON x1.invoiceid = x2.invoiceid 
                                                   WHERE x2.titulodereferencia IS NOT NULL
                                                     AND (e.invoiceid = x1.titulodereferencia OR e.invoiceid = x2.titulodereferencia))) w1
