...
class bCSVColumn
{
    ...
    const TYPE_ESTADO = 9;
    //usados no gnuteca
    const TYPE_LANGUAGE = 10;
    const TYPE_BUY = 11;
    const TYPE_MATERIALTYPE = 12;
    const TYPE_PHYSICALTYPE = 13;
    const TYPE_EXEMPLARYGENDER = 14;
    const TYPE_EXEMPLARYSTATUS = 15;
    const TYPE_RECORDID = 16;
    const TYPE_ISBNISSN = 17;
    ...
    public function setType($type)
    {
        if ( strlen($type) > 0 )
        {
            ...
            //usados no gnuteca
            else if ( $type == self::TYPE_LANGUAGE )
            {
                $this->setMinAndMaxLength(3);
                $this->setRestrictValues(array('SPA', 'RUS', 'POR', 'LAT', 'JPN', 'ITA', 'GER', 'FRE', 'ENG', 'DUT', 'CHI', 'CAT'));
            }
            else if ( $type == self::TYPE_ISBNISSN )
            {
                $this->setMinAndMaxLength(4);
                $this->setRestrictValues(array('ISBN', 'ISSN'));
            }
            else if ( $type == self::TYPE_BUY )
            {
                $this->setMinAndMaxLength(1);
                $this->setRestrictValues(array('C', 'P', 'D'));
            }
            else if ( $type == self::TYPE_MATERIALTYPE )
            {
                $this->setRanges(1, 44);
            }
            else if ( $type == self::TYPE_PHYSICALTYPE )
            {
                $this->setRanges(1, 10);
            }
            else if ( $type == self::TYPE_EXEMPLARYGENDER )
            {
                $this->setRanges(1, 6);
            }
            else if ( $type == self::TYPE_EXEMPLARYSTATUS )
            {
                $this->setRanges(1, 16);
            }
            else if ( $type == self::TYPE_RECORDID )
            {
                $this->setRanges(1, 6);
            }
