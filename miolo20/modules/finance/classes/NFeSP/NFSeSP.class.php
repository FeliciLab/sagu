...
class NFSeSP
{
    public $cnpjPrestador; // = '50677079000168'; // Your CNPJ
    public $ccmPrestador; // = '84630159'; // Your CCM
    private $passphrase; // = 'caepp2015'; // Cert passphrase
    private $pkcs12; //  = 'certificado.pfx';
    ...
    public function __construct($pkcs12, $certDir, $passphrase)
    {
    ...
    private function validateCert($cert)
    {
        try
        {
            ...
            {
                throw new Exception(__METHOD__ . ': Certificado expirado em ' . date('Y-m-d', $certValidDate));
            }
    ...
    private function loadCert()
    {
        try
        {
            ...
            {
                throw new Exception(__METHOD__ . ' : Certificado não pode ser lido. O arquivo esta corrompido, ou em formato inválido ou a senha inválida.');
            }
            $this->X509Certificate = preg_replace("/[\n]/", '', preg_replace('/\-\-\-\-\-[A-Z]+ CERTIFICATE\-\-\-\-\-/', '', $x509CertData['cert']));
            if ( !self::validateCert($x509CertData['cert']) )
            {
                throw new Exception(_M('Não foi possível validar o certificado digital.'));
            }
            if ( !is_dir($this->certDir) )
            {
                if ( !mkdir($this->certDir, 0777) )
                {
                    throw new Exception(__METHOD__ . ': Falha ao criar o diretorio ' . $this->certDir);
                }
            }
            if ( !file_exists($this->privateKey) )
            {
                ...
                {
                    throw new Exception(__METHOD__ . ': Falha ao criar o arquivo ' . $this->privateKey);
                }
            }
            if ( !file_exists($this->publicKey) )
            {
                ...
                {
                    throw new Exception(__METHOD__ . ': Falha ao criar o arquivo ' . $this->publicKey);
                }
            }
            if ( !file_exists($this->key) )
            {
                ...
                {
                    throw new Exception(__METHOD__ . ': Falha ao criar o arquivo ' . $this->key);
                }
            }
            return true;
        }
    ...
    public function start()
    {
        ...
        {
            throw new Exception('Erro de conexão soap. Tente novamente mais tarde. <br> ' . $e->getMessage());
        }
    }
    ...
    private function send($operation, $xmlDoc)
    {
        ...
        {
            throw new Exception('Erro soap: <br>' . $e->getMessage());
        }
    ...
    private function insertTextRPS(NFeRPS $rps, &$body)
    {
        if ( $rps->valorServicos > 0 )
        {
            $line = "2" .
                    ...
                    str_replace("-", "", $rps->dataEmissao) .
                    $rps->tributacao .
                    sprintf('%015s', str_replace('.', '', $rps->valorServicos)) .
