...
abstract class RetornoCNAB400Base extends RetornoBase {
  ...
  protected function processarHeaderArquivo($linha) {
      parent::processarHeaderArquivo($linha);
	...
	protected function processarDetalhe($linha)
        {
            parent::processarDetalhe($linha);
                ...
                $linha = substr($linha, 0, 1) == ' ' ? $linha : " {$linha}";
		...
		$vlinha["data_vencimento"]     = $this->formataData(substr($linha, 147,   6)); //9  Data de vencimento (DDMMAA) (ver nota 6 para convênios de 7 dígitos)
		...
		$vlinha["data_credito"]        = $this->formataData(substr($linha, 176,   6)); //9  Data do crédito (DDMMAA) - ver nota 10
	...
	* @return array<mixed> Retorna um vetor contendo os dados dos campos da linha trailer do arquivo.*/
	protected function processarTrailerArquivo($linha) {
            parent::processarTrailerArquivo($linha);
