...
var _MIOLO_MultiTextField_separator = '] [';
 ...
 *  
 */
function _MIOLO_MultiTextField_Split(value)
{
    return value.substring(1, value.length - 1).split(_MIOLO_MultiTextField_separator);
}
/**
 *  
 */
function _MIOLO_MultiTextField_Join(fields)
{
    var value = '[';
    ...
    for ( var i = 0; i < fields.length; i++ )
    {
        if ( i > 0 )
        {
            value += _MIOLO_MultiTextField_separator;
        }
 ...
 *  
 */
function _MIOLO_MultiTextField_onKeyDown(source, frmObj, mtfName, event, numFields)
{
    ...
    if (source.name.substring(0, len) == name)
    {
        if (key == 13) // enter key
        {
            _MIOLO_MultiTextField_add(frmObj, mtfName, numFields);
            return false;
        }
    }
    ...
    else if (source.name == mtfName)
    {
        ...
        {
            _MIOLO_MultiTextField_remove(frmObj, mtfName, numFields);
            return false;
        }
    }
}
 ...
 *  
 */
function _MIOLO_MultiTextField_onSelect(frmObj, mtfName, numFields)
{
    var list = MIOLO_GetElementById(mtfName);
    ...
    if (i != -1)
    {
        var a = _MIOLO_MultiTextField_Split(list.options[i].text);
 ...
 *  
 */
function _MIOLO_MultiTextField_getInput(frmObj, mtfName, numFields)
{
    var list = MIOLO_GetElementById(mtfName);
    ...
    return _MIOLO_MultiTextField_Join(fields);
}
/**
 *  
 */
function _MIOLO_MultiTextField_add(frmObj, mtfName, numFields)
{
    var list = MIOLO_GetElementById(mtfName);

    var i = list.length;

    var name = _MIOLO_MultiTextField_getInput(frmObj, mtfName, numFields);
 ...
 */
function _MIOLO_MultiTextField_remove(frmObj, mtfName, numFields)
{
    var list = MIOLO_GetElementById(mtfName);
 ...
 * 
 */
function _MIOLO_MultiTextField_modify(frmObj, mtfName, numFields)
{
    var list = MIOLO_GetElementById(mtfName);
    ...
    if (i != -1)
    {
        list.options[i] = new Option(_MIOLO_MultiTextField_getInput(frmObj, mtfName, numFields));
    }
 ...
 * 
 */
function _MIOLO_MultiTextField_moveUp(frmObj, mtfName, numFields)
{
    var list = MIOLO_GetElementById(mtfName);
 ...
 * 
 */
function _MIOLO_MultiTextField_moveDown(frmObj, mtfName, numFields)
{
    var list = MIOLO_GetElementById(mtfName);
