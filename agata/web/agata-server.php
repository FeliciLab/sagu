...
/*
 * open_file
 * Reads a file ans returns as a string
 * by Pablo Dall'Oglio 2005
 */
...
/*
 * save_file
 * receives a string and saves the file
 * by Pablo Dall'Oglio 2005
 */
function save_file($File, $text, $folder = 'reports')
{
    $text = base64_decode($text);
    $AgataPath = get_agata_path();
    if (substr($File,0,1)=='\\' or substr($File,0,1)=='/')
    {
        $File = substr($File,1);
    }
    $FileName = $AgataPath . "/{$folder}/" .  $File;
...
/*
 * dir_list
 * reads a dir entry and returns as an array
 * by Pablo Dall'Oglio 2005
 */
...
# Registra o Serviço
$server->register('dir_list');
$server->register('is_remote');
$server->register('open_file');
$server->register('save_file');
$server->register('file_exists');
$server->register('unlink');
$server->register('is_dir');
