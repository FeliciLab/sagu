...
include('agata-server.conf.php');

/**
 * Agata - Server Side
 * Program to manipulate Registers.
 *
 ...
 *
 * @since
 * Class created on 25/09/2008 (newer version)
 *
 * \b Organization: \n
 * SOLIS - Cooperativa de Solucoes Livres \n
 * The Agata Development Team
 *
 * \b CopyLeft: \n
 * CopyLeft (L) 2008 SOLIS - Cooperativa de Solucoes Livres \n
 *
 * \b License: \n
...
function a_M($msg,$dom='miolo',$p1=null,$p2=null,$p3=null)
{
	return $msg;
}

/**
* Append a string to a file (in a new Line).
*
* @param $filename the file to save
* @param $conteudo the content to add to file
*/
function save_to_txt($filename, $conteudo)
{
...
/**
* Get the database and make a query in it
*
* @param $sql the sql to query
* @param $object if is to return in object (or otherwise array)
*
*/
function query($sql, $object=true, $type='pgsql')
{
    if ($type  = 'pgsql')
    {
        $dbconn = pg_connect('host='.HOST.' dbname='.BASE.' user='.USER.' password='.PASS);
...
//var_dump(query("select userid, login, name, password, email, isAdmin from agtuser;",false) );
...
*
*/
function execute($sql, $type='pgsql')
{

    if ($type= 'pgsql')
    {
        $dbconn = pg_connect('host='.HOST.' dbname='.BASE.' user='.USER.' password='.PASS);
        //$dbconn = pg_connect("host=192.168.100.6 dbname=agataserver user=postgres password=");
    ...
    {
        $conn   = mysql_connect('192.168.100.6' , 'root' , '' );
        $db     = mysql_select_db('agataserver', $conn );
        $result = mysql_query($sql, $conn );
        return    $result;
    }
}
...
/*var_dump(
execute(
"insert into agtuser ( login, name, password, email, isAdmin) values ( 'uhu','Uhuzador',md5('uhu'),'uhu@solis.coop.br', true);"
 ) );*/

/**
* Authenticate LDAP
*
* @param $data->login the user login
* @param $data->password the user password
* @param $data->project the name of the project
*/
function agataAuthenticateLdap($id, $pwd)
{
    $SERVER         = LDAP_SERVER;
    $SERVER_SLAVE   = LDAP_SERVER_SLAVE;
    $PORT           = LDAP_PORT;
    ...
    $r=ldap_bind($ds, $ADMIN, $ADMIN_PASSWORD);

    if (!$r)
    {
	$ds=ldap_connect("$SERVER_SLAVE", "$PORT");
	ldap_set_option($ds, LDAP_OPT_PROTOCOL_VERSION, 3);
	$r=ldap_bind($ds, $ADMIN, $ADMIN_PASSWORD);
    }
    ...
    if ($r)  
    {
	//$sr= ldap_search($ds,"$BASE_DN","(&(objectClass=univates)(codAluno=$id))", array('dn'));
	$sr= ldap_search($ds,"$BASE_DN","(&($FILTER=$id))", array('dn'));
...
/**
* Make the login in agata Database and return the needed objects
*
* @param $data->login the user login
* @param $data->password the user password
* @param $data->project the name of the project
*/
function agataLogin($data)
{
    ...
    {
        if (agataAuthenticateLdap($data->login, $data->password))
        {
            $sql = ' select userid, login, name, password, email, isAdmin from agtuser where login=\''.$data->login.'\'';
        }
    }
    else
    {
	$sql    = ' select userid, login, name, password, email, isAdmin from agtuser where login=\''.$data->login.'\' and password=\''.md5($data->password).'\';';
    }
    $object ->user = query($sql);

    $sql    = 'select projectid, projectname,basename, description,author, date, host, baseuser, pass, type, dict from agtproject where lower(projectname) like lower(\''.$data->project.'\');';
    $object ->project = query($sql);

    $sql     = 'select agttable from agtaccess where userid = (select userid from agtuser where lower(login) like lower(\''.$data->login.'\')  ) and projectid = ( select projectid from agtproject where lower(projectname) like lower(\''.$data->project.'\'));';
...
$data->project  = 'gnuteca3alias';
$data           = serialize($data);
var_dump(unserialize(agataLogin($data)));*/



/**
* List all user with his propertys
*/
function listUser()
{
    $sql = 'select userid, login, name, password, email, isAdmin from agtuser;';
    $result  = query($sql,false);
...
function saveUser($data)
{
    ...
    if ($userData->userid)
    {
        ...
        return updateUser(serialize($data));
    }
    else
    {
        return insertUser(serialize($data));
    }
}
...
{
    $data= unserialize($data);
    $sql = "insert into agtuser (login, name, password, email,  isAdmin) values ('{$data->login}','{$data->name}', '{$data->password}', '{$data->email}',{$data->isadmin});";
    //return serialize($sql);
...
{
    $data = unserialize($data);
    $sql  = "update agtuser set login ='{$data->login}', name ='{$data->name}',email ='{$data->email}', isAdmin = {$data->isadmin} ";
    if ($data->password)
    {
...
{
    $data = unserialize($data);
    $sql = "delete from agtuser where login = '{$data->login}';";
    $result  = execute($sql);
    return serialize($result);
}
...
/**
* List all projects with his propertys
*/
function listProjects()
{
    $sql = 'select projectid, projectname, basename, description, author, date, host, baseuser, pass, type, dict from agtproject;';
    $result  = query($sql,false);
    return serialize($result);
}
...
/**
* Save a project to agata database
*
*/
function saveProject($data)
{
    $data           = unserialize($data);
    $projectData    = unserialize( getProject( serialize($data) ));
    if ($projectData->projectid)
    {
        $data->projectId = $projectData->projectid;
        return updateProject(serialize($data));
    }
    else
    {
        return insertProject(serialize($data));
    }
}
...
{
    $data = unserialize($data);
    $sql = "update agtproject set projectname = '{$data->projectname}', basename = '{$data->basename}', description = '{$data->desc}', author = '{$data->author}', date = '{$data->date}', host= '{$data->host}', baseuser ='{$data->user}' ,pass = '{$data->pass}' , type= '{$data->type}', dict='{$data->dict}' where projectId = '{$data->projectId}'";
    $result = execute($sql);
    return serialize($result);
}
...
{
    $data = unserialize($data);
    $sql = "insert into agtproject (projectname, basename, description, author, date, host, baseuser, pass, type, dict) values('{$data->projectname}', '{$data->basename}', '{$data->desc}', '{$data->author}','{$data->date}', '{$data->host}', '{$data->user}', '{$data->pass}', '{$data->type}', '{$data->dict}');";
    $result = execute($sql);
    return serialize($result);
}

/**
* Get the project Data passing it name
* The data needs to be UNSERIALIZE BEFORE GET
* @param $data->name the name of the project to get
*/
function getProject($data)
{
    $data = unserialize($data);
    $sql = 'select projectid, projectname, basename, description,author, date, host, baseuser, pass, type, dict from agtproject where lower(projectname) = lower(\''.$data->projectname.'\');';
    $result = query($sql);
    return serialize($result);
}
...
function deleteProject($data)
{
    ...
    if ($projectData->projectid)
    {
        $sql = 'delete from agtproject where projectid = '.$projectData->projectid.';';
        $result = execute($sql);
        return serialize($result);
    }
    else
...
{
    $data    = unserialize($data);
    $sql = "delete from  agtaccess where userid = (select userid from agtuser where lower(login)=lower('{$data->user}') limit 1) and projectid = ( select projectid from agtproject where lower(projectname) = lower('{$data->projectName}') limit 1) and lower(agttable) = lower('{$data->access}') ;";
    $result = execute($sql);
    return serialize($result);
}
...
{
    $data    = unserialize($data);
    if ($data->user && $data->projectName && $data->access)
    {
        $sql     = "insert into agtaccess (userid, projectid, agttable) values ( (select userid from agtuser where lower(login) like lower('{$data->user}') ), (select projectid from agtproject where lower(projectname) like lower('{$data->projectName}')) ,'{$data->access}');";
        $result = execute($sql);
    }
    return serialize($result);
}

function listAccess($data)
{
    $data    = unserialize($data);
    $sql     = "select agttable from agtaccess where userid = (select userid from agtuser where lower(login)=lower('{$data->user}') limit 1) and projectid = ( select projectid from agtproject where lower(projectname) = lower('{$data->projectName}') limit 1) ;";
    $result = query($sql, false);
    return serialize($result);
}

# Cria o objeto servidor SOAP
$server = new soap_server;

# Registra os Serviços
$server->register('agataLogin');
$server->register('listUser');
$server->register('saveUser');
$server->register('insertUser');
$server->register('updateUser');
$server->register('deleteUser');
$server->register('getUser');
$server->register('listProjects');
$server->register('saveProject');
$server->register('deleteProject');
$server->register('getProject');
$server->register('insertProject');
$server->register('updateProject');
$server->register('deleteAccess');
$server->register('insertAccess');
$server->register('listAccess');

# Formula a resposta para o servidor SOAP
