...
function cmActionItem (item, isMain, idSub, menuInfo, menuID)
{
	...
	return returnStr + ' onmouseout="cmItemMouseOut (' + param + ')" onmousedown="cmItemMouseUp (' + param + ')"';
}
...
function cmDrawSubMenu (subMenu, prefix, id, nodeProperties, zIndexStart, menuInfo, menuID)
{
	...
	for (i = 5; i < subMenu.length; ++i)
	{
            ...
            {
                if ( item[3] == '_blank' || item[3] == '_new' )
                {
                    str += '<a href="' + item[2]  + '" target="' + item[3] + '">';
                }
                else
                {
                    str += '<a href="' + item[2]  + '" onclick="miolo.doLink(\'' + item[2]  +'\', \'__mainForm\'); return false;">';
                }
            ...
            if (item[0] != null)
            {
                str += '<div class="' + classStr + 'Image">';
                str += item[0].replace(new RegExp("^'|'$", "g"), "");
                str += '</div>';
            }
            else
            {
