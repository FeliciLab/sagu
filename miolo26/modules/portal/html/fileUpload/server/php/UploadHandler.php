...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true) {
        ...
        $module = 'portal';
        $path       = '/var/www/miolo/';
        $classPath  = '/var/www/miolo/classes/';
        
        include("$classPath/mioloconsole.class.php");

        $this->MIOLOConsole = new MIOLOConsole();
        $this->MIOLO = $MIOLOConsole->getMIOLOInstance($path, $module, false);
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    #HACK para receber nome do arquivo no form
                    $_SESSION['file_tmp'][$file_path] = $uploaded_file; 
                    $_SESSION['file_path'][$file_path] = $file_path;
                }
            } else {
        ...
        if ($success) {
            foreach($this->options['image_versions'] as $version => $options) {
                if (!empty($version)) {
                    ...
                    if (is_file($file)) {
                        unlink($file);
                        #HACK para apagar arquivos
                        $_SESSION['file_tmp'][$file] = null; 
                        $_SESSION['file_path'][$file] = null;
                    }
                }
            }
        }
