...
$MIOLO->uses( "classes/alogin.class.php", avinst );

class MAuthLdapUnivates extends MAuth
{
    ...
    public function authenticate($user, $pass, $log=true)
    {
        ...
        if($log && $r)
        {
            $login = new MLogin($info[$i][strtolower($idPerson)][0],
                                $pass,
                                $info[$i][strtolower($idPerson)][0],
                                0);
            $login->setIdPerson( $info[$i][strtolower($idPerson)][0] );
