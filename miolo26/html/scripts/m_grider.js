    ...
    $.Grider = function(table, config) {
        ...
        function setGrider(t)
        {
            ...
            if(config['countRow']) {
                if(config['countRowAdd']) {
                    ...
                    {
                        var ind = index+1;
                        //$(elem).prepend('<td>'+ind+'<input class=\'griderStatus\' id=\''+table.id+'['+(ind-1)+'][status]\'/></td>');
            ...
            if(config['addRow'])
            {
                ...
                $(table).find(".addButton").click(function()
                {
                    addRow();
                });
            }
        ...
        */
        ...
        function addRow()
        {
            ...
            if($(tr).find("input, button, select, textarea, div, table, tr, td, img, a").length > 0)
            {
                $(tr).find("input, button, textarea, select, div, table, tr, td, img, a").each(
                    ...
                    function(index, elem)
                    {
                        ...
                        if(config.formPos !== '')
                        {
                            newName = (elem.name == null || elem.name == '' || elem.name == 'undefined') ? elem.id : elem.name;
                            newName = newName.replace(/\[[0-9]+\]/i, '[' + countRows + ']');
                        }
                        else
                        {
                            newName =(elem.name == null || elem.name == '' || elem.name == 'undefined') ? elem.id : elem.name;
                        ...
                        //adiciona suporte ao componente mcalendarfield do dojo
                        if ( $(elem).attr('class') == 'dijitReset dijitInputInner')
                        {
                        
                            newName = $(elem.parentNode).find('input').eq(1).attr('name');
                            ...
                            calendar.valueNode.name = newName;
                        }
                        
                        $(elem)
                    });
                
                //marca o insert no campo status
                $(tr).find('.griderStatus').eq(0).val('add');
                
                //desmarca todas checkbox e radio
        ...
        function delRow(elem)
        {
            ...
            if ( $(table).find('.mTableRawRow:not(.trRemoved)').length > 1 )
            {
                if(config.rails)
                {
                    ...
                    if(el.length > 0)
                    {
                        ...
                        var value = $(el).val();
                        n1 = name.replace(/^(.*)(\[[0-9]+\])(\[id\])$/, "$1["+ countRows + "]$3");
                        n2 = name.replace(/^(.*)(\[[0-9]+\])(\[id\])$/, "$1["+ countRows +"][_delete]");
                        $(el).remove();
                        $(table).prepend('<span><input type="hidden" name="'+ n1 +'" value="'+ value +'" /><input type="hidden" value="1" name="'+ n2 +'"</span>');
                    }
                }
                
                if (config.hideDelRow)
                {
                    var tr = $(elem).parents('tr').eq(0);
                    var statusField = tr.find('td .griderStatus').eq(0);
                    
                    //caso tenha sido recentemente adicionado remove
                    if ( statusField.val() == 'add')
                    {
                        tr.remove();
                    }
                    /*else
                    {
                        //caso contrário define estado
                        //tr.find('td .griderStatus').eq(0).val('remove');
                        //tr.addClass('trRemoved'); //marca classe css de remoção
                        //tr.hide();
                    }*/
                }
                else
                ...
                if(config['countRow'])
                {
                    rowNumber(); //comentando em função da coluna status
                }
            }
...
Grider =
{
    defaults :
    {
        ...
        delRow: true,
        decimals: 2,
        addRowText: '<a class="addButton" href="#" onclick="return false;">Adicionar colunasss</a>',
        delRowText: '<td><a href="#" onclick="return false;" class="delete">Remover</a></td>',
        countRow: false,
        countRowText: 'Nº',
        countRowCol: 0,
        countRowAdd: false,
